...
var figlet = figlet || (function () {
  ...
  function getSmushingRules (oldLayout, newLayout) {
    ...
    const codes = [[16384, 'vLayout', SMUSHING], [8192, 'vLayout', FITTING], [4096, 'vRule5', true], [2048, 'vRule4', true],
  ...
  function canVerticalSmush (txt1, txt2, opts) {
    ...
    let ii; const len = Math.min(txt1.length, txt2.length)
    let ch1; let ch2; let endSmush = false; let validSmush
  ...
  function verticallySmushLines (line1, line2, opts) {
    let ii; const len = Math.min(line1.length, line2.length)
    let ch1; let ch2; let result = ''; let validSmush
  ...
  function verticalSmush (lines1, lines2, overlap, opts) {
    ...
    let ii; let len; let line; const piece2 = []; let piece3; const result = []
  ...
  function padLines (lines, numSpaces) {
    let ii; const len = lines.length; let padding = ''
  ...
  function getHorizontalSmushLength (txt1, txt2, opts) {
    ...
    let ii; const len1 = txt1.length; const len2 = txt2.length
    const maxDist = len1
    let curDist = 1
  ...
  function horizontalSmush (textBlock1, textBlock2, overlap, opts) {
    let ii; let jj; let ch; const outputFig = []
    let overlapStart; let piece1; let piece2; let piece3; let len1; let len2; let txt1; let txt2
    ...
    for (ii = 0; ii < opts.height; ii++) {
      ...
      const seg1 = txt1.substr(Math.max(0, len1 - overlap), overlap)
      const seg2 = txt2.substr(0, Math.min(overlap, len2))

      for (jj = 0; jj < overlap; jj++) {
        const ch1 = (jj < len1) ? seg1.substr(jj, 1) : ' '
        const ch2 = (jj < len2) ? seg2.substr(jj, 1) : ' '
  ...
  function generateFigTextLines (txt, figChars, opts) {
    let charIndex; let figChar; let spaceIndex; let overlap = 0; let row; let outputFigText; let len; const height = opts.height; const outputFigLines = []; let maxWidth; let prevWidth; let nextFigChars; let figWords = []; let char; let isSpace; let textFigWord; let textFigLine; let tmpBreak
  ...
  const getHorizontalFittingRules = function (layout, options) {
    const props = ['hLayout', 'hRule1', 'hRule2', 'hRule3', 'hRule4', 'hRule5', 'hRule6']
    let params = {}; let prop; let ii
    if (layout === 'default') {
  ...
  me.parseFont = function (fontName, data) {
    ...
    let cNum; let endCharRegEx; let parseError = false
  ...
  me.loadFont = function (fontName, next) {
    ...
    fetch(figDefaults.fontPath + '/' + fontName)
